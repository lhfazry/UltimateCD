...
def total_intersect_and_union_per_sample(results,
                              gt_seg_maps,
                              num_classes,
                              ignore_index,
                              label_map=dict(),
...
def get_key_values(data, key):
    res = []
    for item in data:
        res.append(np.round(np.nanmean(item[key]) * 100, 2))
    return res

def mean_confidence_interval(data, key, confidence=0.95):
    new_data = get_key_values(data, key)
    a = 1.0 * np.array(new_data)
    n = len(a)
...
def eval_metrics(results,
    ...
    if confidence is not None:
        ...
        for total_area_intersect, total_area_union, total_area_pred_label, \
            total_area_label in zip(total_area_intersects, total_area_unions, \
                                    total_area_pred_labels, total_area_labels):
