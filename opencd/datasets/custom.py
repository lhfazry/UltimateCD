...
class CDDataset(Dataset):
    ...
    def load_annotations(self, img_dir, img_suffix, sub_dir_1, sub_dir_2,
        ...
        if split is not None:
            ...
            file_list_1 = self.file_client.list_dir_or_file(
                dir_path=osp.join(img_dir, sub_dir_1),
                list_dir=False,
                suffix=img_suffix,
                recursive=True)
            file_list_2 = self.file_client.list_dir_or_file(
                dir_path=osp.join(img_dir, sub_dir_2),
                list_dir=False,
                suffix=img_suffix,
                recursive=True)

            assert sorted(list(file_list_1)) == sorted(list(file_list_2)), \
                'The images in `sub_dir_1` and `sub_dir_2` are not ' \
    ...
    def pre_pipeline(self, results):
        ...
        results['seg_fields'] = []
        results['img_prefix'] = [osp.join(self.img_dir, self.sub_dir_1), \
            osp.join(self.img_dir, self.sub_dir_2)]
        results['seg_prefix'] = self.ann_dir
        results['format_ann'] = self.format_ann
    ...
    def evaluate(self,
        ...
        if mmcv.is_list_of(results, np.ndarray) or mmcv.is_list_of(
            ...
            ret_metrics, confidences = eval_metrics(
                results,
                gt_seg_maps,
                num_classes,
                self.ignore_index,
