...
class MultiImgResizeToMultiple(object):
...
class MultiImgResize(object):
    ...
    def _resize_img(self, results):
        ...
        if self.keep_ratio:
            ...
            imgs = [
                mmcv.imrescale(img, results['scale'], return_scale=False)
            ...
            imgs = [img_0] + [
                mmcv.imresize(img, results['scale'], return_scale=False)
                for img in results['img'][1:]
            ]
        scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                dtype=np.float32)
        results['img'] = imgs
        results['img_shape'] = imgs[0].shape
        results['pad_shape'] = imgs[0].shape  # in case that there is no padding
...
@PIPELINES.register_module()
class MultiImgRandomFlip(object):
    """Flip the image & seg.
...
@PIPELINES.register_module()
class MultiImgNormalize(object):
    """Normalize the image.
...
@PIPELINES.register_module()
class MultiImgRandomCrop(object):
    ...
    def __call__(self, results):
        ...
        imgs = results['img']
        crop_bbox = self.get_crop_bbox(imgs[0])
        ...
        # crop the image
        imgs = [
            self.crop(img, crop_bbox) for img in results['img']
        ]
...
@PIPELINES.register_module()
class MultiImgRandomRotate(object):
...
class MultiImgRGB2Gray(object):
    ...
    def __call__(self, results):
        ...
        results['img_shape'] = res_imgs[0].shape
...
@PIPELINES.register_module()
class MultiImgAdjustGamma(object):
...
@PIPELINES.register_module()
class MultiImgPhotoMetricDistortion(object):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last. 
    For multiple images, the transformations are different.
...
@PIPELINES.register_module()
class MultiImgRandomCutOut(object):
    """CutOut operation.
...
@PIPELINES.register_module()
class MultiImgExchangeTime(object):
...
@PIPELINES.register_module()
class MultiImgRandomMosaic(object):
