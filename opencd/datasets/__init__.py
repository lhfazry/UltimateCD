from .custom import CDDataset
from .levir_cd import LEVIR_CD_Dataset
from .s2looking import S2Looking_Dataset
from .svcd import SVCD_Dataset
from .cdd import CDD_Dataset
from .whu import WHU_Dataset
from .dsifn_cd import DSIFN_CD_Dataset

__all__ = ['CDDataset', 'LEVIR_CD_Dataset', 'S2Looking_Dataset', 'SVCD_Dataset', 'CDD_Dataset', 'WHU_Dataset', 'DSIFN_CD_Dataset']
